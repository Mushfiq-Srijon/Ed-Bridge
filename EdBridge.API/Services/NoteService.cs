    ...
    public class NoteService
    {
        ...
        private NoteDto MapToDto(Note n)
        {
            ...
            return new NoteDto
            {
                ...
                Tags = n.NoteSubjectTags
                    .Select(nst => nst.SubjectTag.Name)
                    .ToList()
            };
        }
        ...
        public async Task<NoteDetailDto?> GetNoteByIdAsync(
    int id,
    ...
    if (countView && (viewerUserId == null || viewerUserId != note.UserId))
    {
        ...
        await _db.SaveChangesAsync();
            ...
            return new NoteDetailDto
            {
                ...
                Tags = note.NoteSubjectTags
                    .Select(nst => nst.SubjectTag.Name)
                    .ToList(),

                Comments = note.Comments
                    .OrderByDescending(c => c.CreatedAt)
                    .Select(c => new NoteCommentDto
                    {
                        Id = c.Id,
        ...
        public async Task<List<NoteDto>> SearchNotesAsync(
            string query,
            int page = 1,
            int pageSize = 10)
        {
        ...
        public async Task<List<NoteDto>> GetNotesBySubjectAsync(
            int subjectTagId,
            int page = 1,
            int pageSize = 10)
        {
            var notes = await _db.Notes
                .Where(n =>
                    n.NoteSubjectTags.Any(
                        nst => nst.SubjectTagId == subjectTagId
                    )
                )
                .OrderByDescending(n => n.CreatedAt)
                .Skip((page - 1) * pageSize)
        ...
        public async Task<bool> IncrementDownloadAsync(int id)
        {
            ...
            // Only increase the count when a real PDF exists.
        ...
        public async Task<NoteCommentDto?> AddCommentAsync(
            int noteId,
            int userId,
            string text)
        {
        ...
        public async Task<object> RateNoteAsync(
            int noteId,
            int userId,
            int rating)
        {
            ...
            var allRatings = await _db.NoteRatings
